...
public class ExFilePath {
    ...
    public func listSubPaths(recurison:Bool = false) -> [ExFilePath] {
        ...
        let opt: FileManager.DirectoryEnumerationOptions = recurison ? [.skipsHiddenFiles] : [.skipsHiddenFiles, .skipsSubdirectoryDescendants]
        if let itor = FileManager.default.enumerator(at: filePath,
                                                     includingPropertiesForKeys: nil,
                                                     options: opt,
                                                     errorHandler: nil) {
            return itor.allObjects.map{ ExFilePath(($0 as! URL).absoluteString) }
        }
        return []
    }
